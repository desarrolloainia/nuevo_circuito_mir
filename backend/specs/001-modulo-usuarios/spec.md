...
**Actualización**: se añade `nombre` (nombre completo de la persona) como
dato obligatorio del usuario, junto a correo, rol y departamento.
...
organización como usuario, indicando su nombre, su correo, su rol (según los
roles del procedimiento PG09: Detector, Jefe de CLD, Técnico de CLD,
Responsable de resolución, Ejecutor, RSGI) y su departamento.
...
**Independent Test**: Se puede probar dando de alta un usuario con nombre,
correo, rol y departamento válidos y comprobando que queda registrado y
consultable.

**Acceptance Scenarios**:

1. **Given** no existe ningún usuario con ese correo, **When** se registra un
   usuario con nombre, correo, rol y departamento válidos, **Then** el
   usuario queda dado de alta y disponible para su consulta.
...
3. **Given** faltan uno o varios de los cuatro datos (nombre, correo, rol o
...
registrados (individualmente o en listado) para saber su nombre, rol y
departamento, por ejemplo al elegir a quién asignar como responsable de
resolución.
...
1. **Given** existen usuarios registrados, **When** se consulta un usuario
   por su correo, **Then** el sistema devuelve su nombre, correo, rol y
   departamento.
...
Un técnico de CLD corrige el nombre, el rol o el departamento de un usuario
cuando cambian sus datos o de puesto, o da de baja a un usuario que ya no
...
1. **Given** un usuario existente, **When** se actualiza su nombre, su rol o
   su departamento, **Then** las consultas posteriores muestran el nuevo
   valor.
...
- **FR-001**: El sistema DEBE permitir registrar un usuario indicando nombre,
  correo, rol y departamento.
...
- **FR-004**: El sistema DEBE restringir el rol a los roles definidos en el
  ...
  Ejecutor, RSGI).
- **FR-005**: El sistema DEBE rechazar el alta de un usuario si falta el
  nombre, el correo, el rol o el departamento.
...
- **FR-008**: El sistema DEBE permitir actualizar el nombre, el rol y/o el
  departamento de un usuario existente.
...
- **Usuario**: representa a una persona de la organización que participa en
  el procedimiento PG09. Atributos: nombre (nombre completo de la persona),
  correo (identificador único de contacto), rol (uno de los roles fijos del
  PG09), departamento (nombre del departamento al que pertenece), estado
  (activo/inactivo). No tiene contraseña ni credenciales de acceso en esta
  fase.
...
- **SC-001**: Se puede dar de alta un usuario con nombre, correo, rol y
  departamento en menos de 1 minuto.
...
- El correo es el identificador único del usuario a efectos de búsqueda;
  el `id` técnico y el `nombre` no sustituyen a esa identificación.
...
- No se gestionan en esta fase otros datos personales adicionales al nombre y
  al correo (teléfono, foto, etc.); se documentarán como ampliación futura si
  se necesitan.
