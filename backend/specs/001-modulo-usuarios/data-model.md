...
| `nombre` | `str` | Obligatorio (FR-001, FR-005); nombre completo de la persona. |
...
- No puede construirse sin `correo`, `nombre`, `rol` y `departamento`
  válidos (FR-001, FR-005) — la validación ocurre en el `__init__`/value
  objects, no en el router.
...
- `actualizar(nombre=None, rol=None, departamento=None)`: solo puede
  aplicarse sobre un usuario activo; si el usuario está inactivo, no se
  expone actualización vía caso de uso (se da de alta uno nuevo o se
  reactiva en una iteración futura, fuera de alcance de esta spec).
