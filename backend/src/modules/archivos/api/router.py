...
from modules.archivos.application.ports.file_storage import FileStorageNotFoundError, FileStoragePort
from modules.archivos.application.uses_cases.obtener_documentos import (
    obtener_documentos,
)
...
async def obtener_contenido_archivo(
    ...
    try:
        ...
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Archivo no encontrado") from error
    except Exception as error:
        raise HTTPException(status.HTTP_502_BAD_GATEWAY, "No se ha podido leer el archivo") from error

    return Response(
        content=contenido,
        media_type=guess_type(documento.nombre)[0] or "application/octet-stream",
        headers={
            "Content-Disposition": "attachment; filename*=UTF-8''" + quote(documento.nombre, safe=""),
