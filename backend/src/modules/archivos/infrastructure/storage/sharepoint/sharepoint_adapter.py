...
class SharePointFileStorageAdapter(FileStoragePort):
    ...
    async def subir(
        ...
        try:
            drive_item = await self._client.upload_content(
    ...
    def _traducir_error(error: APIError) -> FileStorageError:
        ...
        if status == 404:
            return FileStorageNotFoundError(
                mensaje or "Archivo no encontrado en SharePoint"
            )
        if status in (401, 403):
            return FileStorageAuthError(
                mensaje or "Fallo de autenticacion contra SharePoint"
            )
        if status == 429:
