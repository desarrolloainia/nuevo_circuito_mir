...
class SharePointGraphClient:
    ...
    def _item(self, item_id: str):
        return self._client.drives.by_drive_id(self._drive_id).items.by_drive_item_id(
            item_id
        )
    ...
    async def upload_content(
        ...
        request_configuration = (
            ContentRequestBuilder.ContentRequestBuilderPutRequestConfiguration(
                headers=headers
            )
        )
        ...
        if drive_item is None:
            raise RuntimeError(
                "Microsoft Graph no devolvio el driveItem tras la subida"
            )
        return drive_item

    async def get_documentos(self) -> list[DriveItem]:
        items = (
            await self._client.drives.by_drive_id(self._drive_id)
            .items.by_drive_item_id(self._root_folder_item_id)
            .children.get()
        )
        if items is None:
    ...
    async def download_content(self, item_id: str) -> bytes:
        ...
        if content is None:
            raise RuntimeError(
                "Microsoft Graph no devolvio contenido para el driveItem"
            )
        return content
