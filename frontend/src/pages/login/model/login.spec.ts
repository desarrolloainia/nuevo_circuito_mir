...
it('exige departamento para el registro', () => {
  expect(validateRegistration({ email: 'nuevo@mir.es', role: 'DETECTOR', department: ' ' })).toEqual([
    { name: 'department', message: 'Introduce tu departamento.' }
  ])
  expect(validateRegistration({ email: 'nuevo@mir.es', role: 'DETECTOR', department: 'Calidad' })).toEqual([])
})
